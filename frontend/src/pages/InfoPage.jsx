      ...
      <div className="relative max-w-7xl mx-auto px-6 py-10 overflow-x-clip">
        ...
        <CollapsibleSection id="rilis" title="Apa yang Baru — Rilis v2.7" icon={Sparkles} defaultOpen={true}>
          ...
          <div className="grid md:grid-cols-2 gap-4">
            ...
            <ReleaseCard
              ...
              points={[
                "IDOR ditutup: TTD elektronik, 10 endpoint hapus & 10 transisi status kini ter-scope satker",
                "Jurnal Buku Barang lengkap: revaluasi 204/205, transfer keluar 302, guard anti jurnal ganda",
                "Server bebas render berat: weasyprint + 18 PDF reportlab di-offload ke thread + rate-limit",
                "Backup/restore kokoh: retensi tak menghapus backup manual, safety snapshot ke disk, reindex pencarian otomatis",
              ]}
            />
            <ReleaseCard
            ...
            <ReleaseCard
              ...
              points={[
                "Master Pegawai (impor Excel massal), Referensi Pejabat penanda tangan, Unit Kerja berjenjang berpuncak di tingkat satkernya (Eselon I–V)",
        ...
        <CollapsibleSection id="fitur" title="Fitur & Fungsionalitas (Siklus Penuh BMN)" icon={Layers}>
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            <FeatureCard icon={Package} title="Manajemen Aset Lengkap" description="CRUD aset dengan 45+ field: identitas, perolehan, organisasi, kondisi, GPS, stiker, dokumen" color="blue" items={["Partial update (PATCH diff) — hanya field berubah", "Validasi kode aset 10 digit & register 32 hex", "Barang Serupa: grup per kode + batch edit", "Filter lanjutan multi-pilih: 7 filter boleh banyak nilai sekaligus (ATAU di dalam filter, DAN antar-filter) — ikut ke ekspor, stiker, kartu, laporan & mode luring"]} />
            ...
            <FeatureCard icon={MapPinned} title="Peta Aset & Ekspor GIS" description="Lembar peta interaktif di halaman utama — mengikuti pencarian & filter aktif, jalan penuh saat offline" color="teal" items={["Pin status berwarna + badge kamera dan komentar terpisah + border hijau kelengkapan pengguna/BAST", "Desain pin per aset/massal: judul berikon pin-pensil, 421 ikon dalam 25 kategori + pencarian seluruh katalog, tampil bertahap 72 ikon; huruf/angka, unggah ikon standar, lingkaran & garis tepi, palet dan kode warna HEX; ikut peta dibagikan dan luring", "Geser pin = koordinat tersimpan otomatis (aman konflik)", "Filter Barang Serupa + unduh KML/KMZ/SHP (27 atribut, WGS84)"]} />
            ...
            <FeatureCard icon={FileText} title="15+ Laporan Resmi" description="DBHI (8 tipe, termasuk per klasifikasi Tidak Ditemukan), RHI, BAHI, SP, BA, SPTJM, Surat Koreksi, Daftar Pemegang Aset, Eksekutif, Satker, LHI" color="teal" items={["Kop surat resmi 3 baris + alamat multi-baris, tanda tangan Kuasa Pengguna Barang", "Penandatangan Non-ASN otomatis tanpa baris NIP/NIK (privasi)", "Batch Download ZIP mencakup SEMUA laporan per kegiatan sekali klik", "Eksekutif per Barang Serupa + kolom detail toggle", "PDF Data Aset: biasa atau per kode barang level 1–5, lokasi, pemegang, SPM, Eselon I–V, supplier, perolehan, atau PSP; mengikuti filter dan sort dalam kelompok, termasuk ZIP", "Kartu BMN format KTP (satuan & massal) + tanggal gaya Indonesia"]} />
            <FeatureCard icon={Users} title="Pengguna Melekat-ke + BAST" description="Pengguna aset terstruktur: Individual / Jabatan / Operasional" color="blue" items={["Nama jabatan kondisional (Jabatan)", "Jenis operasional: Unit/Tempat/Tugas atau Ruangan", "Nomor BAST + unggah & preview dokumen BAST"]} />
            ...
            <FeatureCard icon={FileText} title="Tanda Tangan Elektronik" description="Pembubuhan internal via link dengan pemeriksaan operator/admin satker" color="pink" items={["Kanvas goresan mulus atau foto kertas → PNG transparan", "Atur LETAK & UKURAN pembubuhan di pratinjau halaman dokumen", "Tambah/hapus peserta yang belum membubuhkan sebelum final; alasan dan riwayat terhubung ke BAST", "Validasi pengelola serta buka ulang satu penanda tangan tanpa mengulang peserta lain", "Tahan rotasi/reload/jaringan putus (draf tersimpan + coba lagi)", "QR + hash verifikasi publik (NIP di-masking), token sekali-pakai"]} />
            <FeatureCard icon={Users} title="Master SDM & Referensi" description="Master Pegawai, Pejabat penanda tangan, Unit Kerja Eselon I–V" color="blue" items={["Tingkat satker dinyatakan di Master Satker: pohon Unit Kerja berpuncak di sana — Ditjen/Badan Eselon I, Kanwil Eselon II, KPP Pratama/Lapas/Madrasah/Kantor Pertanahan Eselon III–IV (satker mandiri karena memegang DIPA sendiri), jadi tak ada lagi tingkat karangan di atas puncaknya", "Impor Excel massal + validasi rekening/WNI-WNA", "Foto pegawai ber-krop persegi (geser/zoom) + avatar di daftar", "Kartu Pegawai UID e-KTP/NFC: tap kartu → identitas terisi di form aset, BAST, & TTD elektronik (hash aman, tanpa data chip)", "Keterkaitan aset↔pegawai: panel Perlu Serah Terima BMN", "Referensi Akun BAS per makna digit 1–6 (KEP-211/PB/2018)", "Tanggalan RODA (hari·bulan·tahun) berbahasa Indonesia di form pegawai"]} />
            ...
            <FeatureCard icon={Package} title="Persediaan Standar SAKTI" description="45 kode transaksi resmi (M01–M99, K01–K99, P01, H01–H03) + FIFO per layer" color="teal" items={["Daftar Transaksi lintas barang: filter kode/arah/periode + Referensi 45 kode", "Daftar Usang/Rusak/Tak Dikuasai (bahan CaLK) + hapus definitif ber-SK", "Koreksi Nilai proporsional ke layer FIFO (kuantitas tetap)", "7 laporan PDF persis format SAKTI: Persediaan, Posisi Neraca, Mutasi per akun, Per Layer, 3 Daftar kondisi"]} />
            <FeatureCard icon={FileText} title="Rantai Nilai & Penyusutan" description="Riwayat Nilai per aset: perolehan → kapitalisasi/revaluasi → nilai buku" color="purple" items={["Nilai buku per NUP = dasar − akumulasi (mesin sama dgn Laporan Penyusutan)", "Timeline membaca jurnal Buku Barang (202/204/205, SIMAN, edit manual)", "Edit harga manual/massal kini berjurnal — tak ada perubahan nilai senyap", "Masa manfaat baru dari LHIP benar-benar berefek (override per aset)"]} />
