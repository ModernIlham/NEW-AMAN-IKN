...
/**
 * Permukaan air beriak versi KANVAS 2D — cadangan ringan untuk efek cair
 * WebGL (`liquid-effect-animation.jsx`).
 *
 * Dipakai bila WebGL tak tersedia, berkas efeknya gagal dimuat, atau pengguna
 * meminta "kurangi gerak". Beberapa lapis gelombang sinus berkecepatan beda
 * (memberi kesan kedalaman), kilau di puncaknya, dan riak yang lahir dari
 * sentuhan/kursor. Nol permintaan keluar, nol dependensi.
 *
 * @param {Object} props
 * @param {string} [props.className] Kelas wadah (kanvas mengisi penuh).
 * @param {number} [props.lapis] Jumlah lapisan gelombang (2–6 wajar).
 * @param {boolean} [props.diam] Gambar satu bingkai lalu berhenti.
 */
export function AirKanvas2D({ className, lapis = 4, diam = false }) {
  ...
  useEffect(() => {
    ...
    function gambar(t) {
      ...
      for (let i = riak.length - 1; i >= 0; i--) {
        ...
        // dan melempar; gejalanya: efeknya mati total begitu kursor digerakkan.
    ...
    function mulai() {
      // Lebar/tinggi nol = elemen sedang `display:none`. Menjalankan loop di
      // keadaan itu membakar CPU tanpa satu piksel pun sampai ke layar.
      if (berhenti || diam || lebar <= 0 || tinggi <= 0 || rafId) return;
      rafId = window.requestAnimationFrame(bingkai);
    }
    function jeda() {
