...
/**
 * Pemilih tanggal RODA (hari · bulan · tahun) — konversi JSX dari komponen
 * `date-wheel-picker` (21st.dev) yang diberikan pemilik proyek; proyek ini
 * murni JSX (components.json `"tsx": false`).
 *
 * Penyesuaian dari aslinya (bukan gaya, tapi keharusan):
 *  1. Gradien pudar atas/bawah memakai `hsl(var(--popover))` — token warna
 *     aplikasi ini adalah TRIPLET HSL (mis. `210 20% 98%`), sehingga
 *     `var(--background)` mentah seperti di aslinya bukan warna yang sah;
 *     dipilih --popover karena roda ini hidup di dalam Popover/Dialog.
 *  2. Nama bulan bawaan `id-ID` (locale bisa dioper).
 *  3. `z-5` aslinya bukan kelas Tailwind → `z-[5]`.
 */
...
function getMonthNames(locale) {
  const formatter = new Intl.DateTimeFormat(locale || "id-ID", { month: "long" });
...
function WheelItem({
  item, index, y, itemHeight, visibleItems, centerOffset, isSelected,
  disabled, onClick,
}) {
  const itemY = useTransform(y, (latest) => index * itemHeight + latest + centerOffset);
  const rotateX = useTransform(
    itemY, [0, centerOffset, itemHeight * visibleItems], [45, 0, -45]);
  const scale = useTransform(
    itemY, [0, centerOffset, itemHeight * visibleItems], [0.8, 1, 0.8]);
  const opacity = useTransform(
    itemY,
    [0, centerOffset * 0.5, centerOffset, centerOffset * 1.5,
...
function WheelColumn({
  items, value, onChange, itemHeight, visibleItems, disabled, className,
  ariaLabel,
}) {
    ...
    >
      <div
        ...
        style={{
          height: centerOffset,
          background:
            "linear-gradient(to bottom, hsl(var(--popover)) 0%, transparent 100%)",
        }}
        aria-hidden="true"
      />
      <div
        ...
        style={{
          height: centerOffset,
          background:
            "linear-gradient(to top, hsl(var(--popover)) 0%, transparent 100%)",
        }}
        aria-hidden="true"
      />
...
const DateWheelPicker = React.forwardRef(
  (
    ...
    const handleYearChange = React.useCallback((yearIndex) => {
      isInternalChange.current = true;
      setDateState((prev) => {
    ...
    React.useEffect(() => {
      if (value && !isInternalChange.current) {
        ...
        if (
          ...
          setDateState({ day: valueDay, month: valueMonth, year: valueYear });
        }
      }
