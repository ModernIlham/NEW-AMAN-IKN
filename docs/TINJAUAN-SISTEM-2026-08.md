...
| U15 | **Callback toolbar stabil; pemuatan ganda saat mengurutkan dihapus** — [PR #1042](https://github.com/ModernIlham/NEW-AMAN-IKN/pull/1042), [PR #1043](https://github.com/ModernIlham/NEW-AMAN-IKN/pull/1043) | Diperiksa ulang 2026-09-09: `SearchInput` sudah memiliki debounce lokal. PR #1042 menstabilkan `refreshData`, buka peta, dan cetak stiker; PR #1043 membuktikan dua kontrol urutan masih memuat nilai lama langsung sebelum efek membaca pilihan baru | `usePenyegaranAset` tetap membaca parameter **dan** pembaca data terbaru sesudah commit React; hook dipakai halaman/timer/form. Toolbar kini hanya mengubah state: prop `refreshData` dihapus, efek halaman menjadi satu pemicu perubahan urutan. 6 uji interaksi tambahan membuktikan dua panggilan lama menjadi satu pemuatan daftar per pilihan pada desktop maupun HP/tablet, ditambah penjaga dependency `sortBy`. Bukan klaim semua komponen memo atau seluruh balapan respons telah ditangani; tidak ada klaim waktu tanpa pengukuran | S |
| U16 | **`React.lazy(PhotoLightbox)` tidak menghasilkan potongan** — belum diubah | `DashboardPage.jsx` lazy, tetapi impor statis tetap ada di `AssetGalleryView.jsx` dan `AssetMapFullView.jsx` (diperiksa ulang 2026-09-09) | **Mengubah satu saja tidak cukup**, tetapi mengganti keduanya dengan lazy saja berisiko merusak pembukaan pertama foto saat luring: `public/service-worker.js` tidak melakukan precache chunk hashed, hanya menyimpannya saat pernah dimuat online. Rancang jaminan ketersediaan chunk dan uji online → luring dahulu. `DashboardPage` sendiri sudah lazy; manfaat pemisahan perlu diukur pada pemuatan dasbor, bukan diklaim sebagai pengurangan potongan login. Ukuran potongan tinjauan lama bukan ukuran build terkini | S |
| U17 | **Perluasan uji render frontend — bertahap** | Angka 724 uji/satu berkas render adalah keadaan tinjauan awal, bukan repo terkini. [PR #1043](https://github.com/ModernIlham/NEW-AMAN-IKN/pull/1043) menambah 6 uji interaksi pengurutan pada toolbar dan Select Radix sebenarnya, memakai hook filter/penyegaran produksi | Pertahankan penjaga wiring berbasis-teks sebagai pelengkap, bukan pengganti uji perilaku. Uji baru membuktikan cacat sebelum memperbaikinya (4 gagal pada kode lama) dan memeriksa hasil satu pilihan, beberapa pilihan berurutan, serta memilih ulang nilai aktif. Perluasan cakupan komponen lain tetap dikerjakan per fitur; ini bukan klaim semua alur UI sudah tercakup | S ×3 |
...
| U26 | **Ditangani PR #1041 (2026-09-08):** plafon unggah kategori/PDF | `unggahan_utils.py`, `routes/categories.py`, `routes/pdf_compress.py` | Kategori 10 MB (termasuk validasi pemilih file/seret-lepas); PDF tetap 25 MB. Pembacaan aplikasi berhenti pada batas + 1 byte sebelum parser/job/penyedia, pesan HTTP 400 jelas. **Bukan vektor OOM** (lihat Bagian 4), bukan pengganti batas request nginx. |
...
Urutan awal tinjauan ini (lihat status terkini pada baris masing-masing): **S4** (pindahkan arsip backup ke luar VPS — nilai
ketahanannya paling tinggi dari gelombang ini) → **U15** (callback toolbar; ditindaklanjuti
PR #1042 dengan koreksi temuan lama) → **S13** (jitter + TTLCache reconnect) → **S9** (`asset_created`
