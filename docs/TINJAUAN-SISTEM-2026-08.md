...
| U15 | **Callback toolbar stabil; cakupan dibatasi pada dasbor** — [PR #1042](https://github.com/ModernIlham/NEW-AMAN-IKN/pull/1042) | Diperiksa ulang 2026-09-09: `SearchInput` sudah memiliki debounce lokal; tiga prop toolbar yang belum stabil adalah `refreshData`, buka peta, dan cetak stiker | Ketiganya distabilkan. `usePenyegaranAset` membaca parameter **dan** `doFetch`/`doFetchStats` terbaru sesudah commit React, sehingga timer/penutupan form tetap memakai konteks kegiatan/filter/snapshot terbaru. `useCallback([])` saja dengan closure lama **tidak cukup**. 8 uji perilaku + 2 penjaga wiring; 3 peringatan dependency berkurang. Bukan klaim semua 26 komponen telah dioptimalkan atau tiap ketukan sebelumnya merender toolbar; tidak ada klaim waktu tanpa pengukuran | S |
