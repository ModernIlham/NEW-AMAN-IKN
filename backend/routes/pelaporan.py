...
from pelaporan_utils import (
    STATUS_PERIODE, kunci_unik_periode, label_periode_pelaporan,
    rekap_periode, validate_buka_periode, validate_kunci_periode,
    validate_periode,
)
...
async def daftar_periode(_user: dict = Depends(require_user)):
    """Register periode pelaporan (terbaru dulu) + rekap."""
    items = [p async for p in db.periode_pelaporan.find({}, {"_id": 0})
             .sort([("tahun", -1), ("semester", -1)]).limit(200)]
    return {"items": items, "ringkasan": rekap_periode(items),
            ...
            "catatan": (
                ...
                "beralasan dan tercatat pada riwayat.")}
